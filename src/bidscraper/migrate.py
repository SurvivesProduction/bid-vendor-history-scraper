"""Migration runner for bidscraper's packaged SQL migrations.

Migrations live under `bidscraper/migrations/*.sql` so they ship inside
...
(e.g. the full/paid overlay) that installs `bidscraper` as a dependency
and wants to run the same migrations without duplicating the SQL.
"""
...
def iter_migration_sql() -> list[tuple[str, str]]:
    ...
    migrations_dir = resources.files("bidscraper").joinpath("migrations")
    entries = sorted(
...
def run_migrations(dsn: str | None = None) -> None:
    ...
    if not migrations:
        raise RuntimeError("No migrations found in the bidscraper package.")
...
def main() -> int:
    ...
    # .env (see bidscraper.config's load_dotenv() comment for why) -- it
